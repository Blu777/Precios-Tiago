...
async function run() {
  // Buscar changomas para Coca Cola 2.25L
  const rs = await client.execute(`
    ...
    WHERE sp.supermercado_id = 'changomas' 
      AND (pm.nombre_estandarizado LIKE '%COCA%2250%' OR pm.nombre_estandarizado LIKE '%COCA%2.25%')
  `);
  console.log(rs.rows);
}

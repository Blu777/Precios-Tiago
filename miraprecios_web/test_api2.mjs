...
async function run() {
  const rs = await client.execute(`
    ...
    WHERE pm.ean IN ('7790895000997', '12110101009')
  `);
  ...
  for (const prod of productos) {
      ...
      for (const group of groupedProducts) {
          ...
          console.log(`Similarity between ${prod.nombre_estandarizado} and ${group.nombre_estandarizado}: ${sim}`);
          if (sim >= 0.50) {
      ...
      if (!merged) {
          groupedProducts.push({...prod, tokens: prodTokens});
      }
  }
